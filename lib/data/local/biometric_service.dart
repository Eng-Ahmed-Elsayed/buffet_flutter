...
// Referenced from the class doc below; the guard is what actually detects a
// changed enrolment.
import 'biometric_enrolment_guard.dart';
...
/// Wraps `local_auth`.
///
/// **Biometric-changed is handled, but not here.** `local_auth` cannot report
/// it — it authenticates without a `CryptoObject`, so the Keystore key Android
/// would invalidate is never involved. [BiometricEnrolmentGuard] creates such a
/// key deliberately and the auth controller checks it before every unlock.
///
